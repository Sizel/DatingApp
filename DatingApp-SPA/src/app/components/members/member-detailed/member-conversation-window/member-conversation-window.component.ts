...
export class MemberConversationWindowComponent implements OnInit {
  ...
  ngOnInit() {
    this.messagesService.connection.on('ReceiveMessage', message => {
  ...
  loadMessages() {
    this.messagesService
      .getConversation(this.auth.decodedToken.nameid, this.user.id)
      .subscribe(
        (messages) => {
  ...
  sendMessage() {
    ...
    this.messagesService
      .sendMessage(this.messageToSend, this.auth.decodedToken.nameid)
      .subscribe(
        (message: Message) => {
          this.messages.unshift(message);
        },
